...
@section('content')
    ...
    <div id="content" class="container-fluid">
        <div class="card">
            ...
            <div class="card-body">
                <small class="font-italic"> Có tất cả {{ $count['all'] }} sản phẩm trong hệ thống </small>
                <div class="analytic">
                            ...
                            @if ($products->total() > 0)
                                @foreach ($products as $product)
                                    ...
                                    <tr>
                                        ...
                                        <td scope="row">{{ $t }}</td>
                                        <td><img  src="{{ url($product->product_thumb) }}" width="100" height="80"
                                                alt=""></td>
                                        <td><a href="">{{ $product->name }}</a>
                                        </td>
                                        <td>{{ number_format($product->original_price, 0, 0, ',')}}đ</td>
                                        <td>{{ number_format($product->price_sale, 0, 0, ',') }}đ</td>
                                        ...
                                        @if (request()->input('status') == 'trash')
                                            <td>{{ $product->deleted_at->format('H:i:s d-m-Y') }}</td>
                                        @endif
                                        <td>
                                            @if (request()->input('status') == 'trash')
                                            @else
